...
function AuthButton({ icon, label, bgColor, textColor, borderColor, onPress }: {
  ...
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.8}
      style={[abStyles.btn, { backgroundColor: bgColor, borderWidth: borderColor ? 1 : 0, borderColor: borderColor ?? 'transparent' }]}>
      {icon}
      <Text style={[abStyles.btnText, { color: textColor }]}>{label}</Text>
    </TouchableOpacity>
  );
}
const abStyles = StyleSheet.create({
  btn: { width: '100%', height: 56, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 12, borderRadius: 4 },
  btnText: { fontFamily: 'Manrope', fontSize: 14, fontWeight: '600' },
});
...
export function SignInScreen() {
  ...
  const handleSendEmailOtp = async () => {
    ...
    try { await signInWithEmail(email.trim()); setAuthTarget(email.trim()); setOtpSent(true); } catch {}
  };
      ...
      <SafeAreaView style={{ flex: 1 }} edges={['top']}>
        ...
        <View style={styles.content}>
          <View style={styles.branding}>
            ...
            <Text style={styles.brandSub}>Back on the tee</Text>
          </View>
          <View style={styles.buttons}>
            ...
            <View style={styles.separator}><View style={styles.separatorLine} /><Text style={styles.separatorText}>or</Text><View style={styles.separatorLine} /></View>
...
const styles = StyleSheet.create({
  ...
  separator: { flexDirection: 'row', alignItems: 'center', gap: 16, paddingVertical: 16 },
  separatorLine: { flex: 1, height: 1, backgroundColor: '#3f4943', opacity: 0.2 },
  separatorText: { fontFamily: 'Manrope', fontSize: 10, color: '#bec9c1', letterSpacing: 3, textTransform: 'uppercase', opacity: 0.5 },
});
